---
name: sweep
description: Batch simplify-pass over recent commits — groups by repo area, runs /simplify 2 batches at a time, dispatches Sonnet fix agents for each batch. Invoke as `/sweep` (last 30 commits) or `/sweep 24h` / `/sweep 50` / `/sweep <sha>`.
user-invocable: true
---

# sweep — batch simplify pass on recent commits

Collects recent commits, groups them by repo area into reasonable batches, runs `/simplify` on 2 batches at a time, then dispatches Sonnet fix agents based on the findings.
...
After each pair of `/simplify` runs, dispatch **Sonnet** fix agents — one per batch — in parallel:

```
Agent({
  subagent_type: "general-purpose",
  model: "sonnet",
  prompt: "Apply these fixes to <batch files>: <findings from /simplify>. Run tests after. Commit if tests pass."
})
```

Give each fix agent the specific findings (file:line + description), not the full simplify output. Fix agents should:
...
- Run `uv run --package <pkg> pytest <pkg>/tests/ -x -q --timeout=30` and commit if green
...
- **Models**: Sonnet for all fix agents. Never Haiku for code review.
