...
4. **Troubleshoot:**
   ...
   - **Exit code 2** — configuration error (`TRANSCRIPT_ARCHIVE_DIR` unset/unreachable, a machine id that sanitizes to empty, or an identity-handshake refusal). For the identity case, the stderr message names which of the two failure modes it is (machine-id collision vs. unmounted/wrong destination) and whether `--adopt-archive` applies — see `tools/transcript-archive/README.md#identity-handshake-failure-modes`. Don't pass `--adopt-archive` reflexively; only after confirming (not guessing) it's a lost-local-state or rebuilt-archive situation, not an active collision between two machines.
   - `install.sh` itself refuses (exit 1, before ever registering the schedule) if the destination `<machine-id>` directory already carries a mismatched identity — it runs the same handshake check as a pre-flight, so a colliding `--machine-id` never gets a recurring job installed in the first place.
