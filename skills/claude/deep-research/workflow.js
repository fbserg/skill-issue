export const meta = {
  ...
  phases: [
    ...
    { title: 'Search',    detail: '3 parallel WebSearch agents, one per angle' },
    { title: 'Fetch',     detail: 'URL-dedup, fetch top 8 sources, extract falsifiable claims' },
    { title: 'Verify',    detail: '2-vote adversarial verification per claim (both must refute to kill)' },
    { title: 'Synthesize',detail: 'Merge semantic dupes, rank by confidence, cite sources' },
  ],
}

const VOTES_PER_CLAIM       = 2
const REFUTATIONS_REQUIRED  = 2   // both votes must refute; one dissent = survives
const MAX_FETCH             = 8
...
const scope = await agent(
  ...
  '## Question\n' + QUESTION + '\n\n' +
  '## Task\n' +
  'Generate 3 distinct web search queries that together cover the question from different angles:\n' +
  '- broad/primary  · academic/technical or recent news  · contrarian/skeptical or practitioner\n\n' +
...
const SEARCH_PROMPT = (angle) =>
  ...
  '## Task\nUse WebSearch with the query above. Return the top 4-5 most relevant results.\n' +
...
const VERIFY_PROMPT = (claim, v) =>
  ...
  'Be SKEPTICAL. Try to REFUTE this claim. Both votes must refute to kill it.\n\n' +
  '## Research question\n' + QUESTION + '\n\n' +
  '## Claim under review\n"' + claim.claim + '"\n\n' +
  ...
  '2. WebSearch for contradicting evidence — does any credible source dispute this?\n' +
  '3. Is the source quality sufficient for the claim\'s strength?\n' +
  '4. Is the claim outdated?\n' +
  '5. Is this marketing fluff / press release / cherry-picked benchmark?\n\n' +
  '**refuted=true** if: unsupported / contradicted / low-quality source / outdated / marketing.\n' +
  '**refuted=false** ONLY if: well-supported, current, source quality matches claim strength.\n' +
  'Default to refuted=true if uncertain.\n\nStructured output only. Evidence MUST be specific.'

// ─── Pipeline: search → dedup → fetch ───
const searchResults = await pipeline(
  ...
  searchResult => {
    ...
    const novel = sorted.filter(r => {
      ...
      if (fetchSlots <= 0 && relRank[r.relevance] >= 1) { budgetDropped.push({ ...r, angle: searchResult.angle }); return false }
...
if (rankedClaims.length === 0) {
  return {
    question: QUESTION, summary: 'No claims extracted.',
    findings: [], sources: allSources.map(s => ({ url: s.url, quality: s.sourceQuality })),
    stats: { angles: scope.angles.length, sources: allSources.length, claims: 0, dupes: dupes.length },
  }
}

// ─── Verify: 2-vote adversarial (barrier — needs full claim pool) ───
phase('Verify')
...
const report = await agent(
  ...
  '**Question:** ' + QUESTION + '\n\n' +
  confirmed.length + ' claims survived 2-vote adversarial verification. Merge semantic duplicates and synthesize.\n\n' +
  '## Confirmed claims\n' + block + '\n' + killedBlock + '\n\n' +
  '## Instructions\n' +
  '1. Merge claims that say the same thing — combine their sources.\n' +
  '2. Group into coherent findings that directly address the research question.\n' +
  '3. Assign confidence: high (multiple primary sources, unanimous votes), medium (secondary or split), low (single/blog).\n' +
  '4. Write a 3-5 sentence executive summary answering the question.\n' +
  '5. Note caveats: uncertainty, weak sources, time-sensitivity.\n' +
  '6. List 2-3 open questions that emerged but weren\'t answered.\n\nStructured output only.',
  { label: 'synthesize', schema: REPORT_SCHEMA }
)

if (!report) {
  return {
    question: QUESTION, summary: 'Synthesis skipped — returning ' + confirmed.length + ' verified claims unmerged.',
    findings: [],
...
return {
  ...
  stats: {
    angles: scope.angles.length, sourcesFetched: allSources.length,
