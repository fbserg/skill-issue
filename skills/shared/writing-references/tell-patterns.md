...
This is a diagnostic lens, not a
...
A concrete **Before / After** pair is kept only for patterns 15, 31, and 32 —
elsewhere it invented a figure or fact absent from the Before line, which is
itself the fabrication this catalog exists to catch. Fix the underlying
thought using the Detect and Why notes instead of manufacturing an example.
