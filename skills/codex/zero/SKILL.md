---
name: zero
description: "Zero out a repo: commit pending work, merge open PRs and every local branch into main, drop worktrees, delete branches, push main, and report open issues."
user-invocable: true
allowed-tools: Bash
---
...
# all local branches except main, with upstream tracking status
...
git status --short
```
If `.git/index.lock` exists, check for a live Git process before continuing:
```bash
ps -ef | grep '[g]it'
ls -l .git/index.lock
```
...
Prefer the repository's documented main-push/deploy command when one exists, for example `just push-main`. Otherwise use:
```bash
...
- Conflicts are part of the work: inspect both sides, preserve the good new code, resolve, validate, commit, then continue.
