...
### Changed
- `resolve-issue`: add parallel fan-out patterns. New "spine vs. fan-out" model — sequential judgment-bearing phases stay Agent calls; the two genuine fan-out phases run as parallel Agent calls (with the Workflow tool as an optional upgrade where it's available, since it's opt-in and absent in headless runs). Step 3 review is now a parallel 3-lens panel (correctness / security & robustness / tests-actually-assert) emitting HANDOFF blocks, with cross-lens dedup and re-review cycles narrowed to surviving lenses; Step 1 gains an optional tier-3 plan panel. Lanes stay on Sonnet (`model: "sonnet"`), convergence may escalate. Mechanism smoke-tested against a synthetic diff — confirmed lens diversity and that cross-lens overlap requires dedup; not yet exercised against a real tier-3 issue with criteria, tests, and a multi-file diff
