...
### Changed
- `resolve-issue`: sharpen the review step and tighten the orchestrator invariant without adding machinery. The Step 3 reviewer now explicitly covers three angles in one pass — correctness, security & robustness, and whether the new tests actually assert (survive the negative control) — closing the original single-reviewer's blind spots without a multi-agent panel. Clarified that handoff blocks may name files and describe findings in prose but never carry source lines or diffs. (An interim revision added Workflow-tool panels for review and tier-3 planning; removed as over-engineering — a capable reviewer covers the three angles directly.)
